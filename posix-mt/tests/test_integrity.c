...
void test_tier1_tier2_segregation() {
    ...
    void* p1 = proteus_memalign(0, 16); // Tier 1
    void* p2 = proteus_memalign(0, 32); // Tier 2
...
void test_tier3_splitting() {
    ...
    void* p1 = proteus_memalign(0, large_size);
    assert(p1 != NULL);
    ...
    void* p2 = proteus_memalign(0, 128);
    assert(p2 == p1); // Should reuse the exact same starting address
    
    void* p3 = proteus_memalign(0, 128);
...
void test_immediate_coalescing() {
    ...
    void* p1 = proteus_memalign(0, 512);
    void* p2 = proteus_memalign(0, 512);
    void* p3 = proteus_memalign(0, 512);
    ...
    void* p_giant = proteus_memalign(0, 1500);
...
void test_swiss_cheese_exhaustion() {
    ...
    for (int i = 0; i < num_blocks; i++) {
        blocks[i] = proteus_memalign(0, 64); // Tier 3 sizing
    }
    ...
    void* giant = proteus_memalign(0, 1024 * 1024); 
    assert(giant != NULL);
